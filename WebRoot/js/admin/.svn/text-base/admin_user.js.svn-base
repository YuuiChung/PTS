		...
		success : function(data){
			if(data === "true"){
				...
				if(changeToStat === 'normal')
				{
					...
					params += "&sendMailSubject=" + getSafeParam("[项目进度跟踪系统(PTS)]帐户激活通知");
					params += "&sendMailContent=" + getSafeParam("您在项目进度跟踪系统(PTS)中的帐户己激活！<br/><a href='" + WEB_ROOT_URL+ "'>立即进入</a>");
...
function addUser()
{
	...
	$.ajax({
		...
		success :function(response){
			...
			params += "&sendMailSubject=" + getSafeParam("[项目进度跟踪系统(PTS)]帐户新建通知");
			params += "&sendMailContent=" + getSafeParam("管理员为您新建了项目进度跟踪系统(PTS)帐户！<br/>用户名：" + email + "<br/>密码:" + password + "<br/><a href='" + WEB_ROOT_URL+ "'>点击进入项目进度跟踪系统(PTS)</a>");
